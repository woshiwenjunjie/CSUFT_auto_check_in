...
2. 正常执行 → 返回 run_checkin 的 result
3. run_checkin 抛异常 → 捕获并通知
"""
...
class TestMainHandler:
    ...
    def test_normal_execution_returns_result(self):
        """正常流程返回 run_checkin 结果"""
        expected = {"status": "ok", "msg": "正常", "date": "2026-06-12"}
        with patch("handler.run_checkin", return_value=expected):
    ...
    def test_exception_caught_and_notified(self):
        """异常时发送通知并返回 error 状态"""
        with patch("handler.run_checkin", side_effect=RuntimeError("API 不可达")):
