...
> 难度：⭐⭐（简单）
>
> 注：Reaple 原名 HTTPCanary，现已更名。旧版仍叫 HTTPCanary，用法完全相同。

---

## 1. 概述

**Reaple**（原 HTTPCanary）是一款 Android 抓包工具，利用手机本地 VPN 拦截 HTTP/HTTPS 流量。
...
4. 根据提示操作（Android 7+ 会提示去系统设置安装）
   ...
   - Android 11+：可能需要 ADB 命令推送（Reaple 内点"移动证书"会给出命令）
5. 证书安装成功后返回 Reaple

> ⚠️ 部分高版本 Android 需要 root 才能安装到系统信任区。如果 Reaple 的内置方案不行，可以试试在设置中启用 **"Reaple 根证书"** 的兼容模式。
...
**Q: Reaple 闪退？**
A: 部分国产 ROM 对 VPN 权限有特殊限制。去系统设置中给 Reaple 所有权限（自启、后台运行、省电无限制）。
...
**Q: 下载的还是 HTTPCanary 不是 Reaple？**
A: 正常。改名后应用商店和官网推送更新需要时间，旧名 APK 安装后打开就是 Reaple。
